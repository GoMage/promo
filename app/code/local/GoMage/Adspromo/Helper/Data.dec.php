...
class GoMage_Adspromo_Helper_Data extends Mage_Core_Helper_Abstract{
	...
	public function a($k, $c = 0, $s = ''){
        ...
        curl_setopt($ch, CURLOPT_POSTFIELDS, 'key='.urlencode($k).'&sku=promo&domains='.urlencode(implode(',', $this->getAllStoreDomains())).'&ver='.urlencode('1.1'));
	...
	public function notify(){
        ...
        try{
	        ...
	        curl_setopt($ch, CURLOPT_POST, true);
	        curl_setopt($ch, CURLOPT_POSTFIELDS, 'sku=promo&timestamp='.$timestamp.'&ver='.urlencode('1.1'));
	        curl_setopt($ch, CURLOPT_TIMEOUT, 30);
	        curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
