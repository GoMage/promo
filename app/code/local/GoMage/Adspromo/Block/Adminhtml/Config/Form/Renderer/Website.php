...
class GoMage_Adspromo_Block_Adminhtml_Config_Form_Renderer_Website extends Mage_Adminhtml_Block_System_Config_Form_Field
{
    ...
    protected function _getElementHtml(Varien_Data_Form_Element_Abstract $element)
    {
    	...
    	$info = Mage::helper('gomage_adspromo')->ga();
    	...
    	foreach (Mage::app()->getWebsites() as $website) {
            ...
            $element->setValue($id);
            $element->setClass('gomage-adspromo-available-sites');
        ...
        $html .= '
        	...
        	function updateGomageAdspromoWebsites(){
        		
        		$("'.$idprefix.'_diasbled").disabled = "disabled";
        		
        		if($$(".gomage-adspromo-available-sites:checked").length >= '.intval($info['c']).'){
    				$$(".gomage-adspromo-available-sites").each(function(e){
    					if(!e.checked){
    						e.disabled = "disabled";
    					}
    				});
    				...
    				if($$(".gomage-adspromo-available-sites:checked").length == 0){
